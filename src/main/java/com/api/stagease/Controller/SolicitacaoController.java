...
import com.api.stagease.Service.SolicitacaoService;
...
public class SolicitacaoController {
    ...
    public ResponseEntity<?> getByIdRequest(@RequestParam("id") final Long id) {
        final SolicitacaoEntity entity = this.repository.findById(id).orElse(null);
        return entity == null ? ResponseEntity.badRequest().body("Esse registro não existe") : ResponseEntity.ok(entity);
    }

    @PostMapping
    public ResponseEntity<?> create(@RequestBody SolicitacaoDTO dto) {
        try {
            service.create(dto);
        } catch (Exception e) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable("id") final Long id, @RequestBody final SolicitacaoDTO dto) {
        try {
